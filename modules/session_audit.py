...
from .utils import c, header, run_cmd
...
def show_last(limit: int = 20) -> list:
    ...
    for e in entries:
        user_col = (
            c("green", f"{e['user']:<12}")
            if e["user"] not in ("reboot", "shutdown")
            else c("dim", f"{e['user']:<12}")
        )
        ip_col  = (
            c("yellow", f"{e['ip']:<20}")
            if re.match(r"\d+\.\d+", e["ip"])
            else f"{e['ip']:<20}"
        )
    ...
    ext_logins = [
        e for e in entries
        if re.match(r"\d+\.\d+", e["ip"]) and e["user"] not in ("reboot", "shutdown")
    ]
    if not ext_logins:
...
def show_lastb(limit: int = 20) -> list:
    ...
    for e in entries:
        ...
        ip_col   = (
            c("red",    f"{e['ip']:<20}") if attempts > 5
            else c("yellow", f"{e['ip']:<20}")
        )
...
def show_active_sessions() -> list:
    ...
    for e in w_entries:
        from_col = (
            c("yellow", f"{e['from_ip']:<20}")
            if re.match(r"\d+\.\d+", e["from_ip"])
            else f"{e['from_ip']:<20}"
        )
...
def detect_anomalies(last_entries: list, lastb_entries: list, active: list) -> list:
    ...
    for ip, cnt in ip_fails.items():
        if cnt > 10:
            anomalies.append(
                f"🔴  Brute-force detected: {cnt} failed attempts from {ip}"
            )
    ...
    for e in last_entries:
        if e["ip"] in fail_ips and re.match(r"\d+\.\d+", e["ip"]):
            anomalies.append(
                f"🟠  Successful login from IP with prior failed attempts: "
                f"{e['ip']} (user: {e['user']})"
            )
    ...
    for e in active:
        if re.match(r"\d+\.\d+", e.get("from_ip", "")):
            anomalies.append(
                f"🟡  Active session from external IP: "
                f"{e['from_ip']} (user: {e['user']})"
            )
