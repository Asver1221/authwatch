...
def get_real_users() -> list[dict]:
    """Return all accounts from /etc/passwd (real and system)."""
    users = []
...
def check_sudoers(findings: list, verbose: bool = True) -> None:
    ...
    if os.access("/etc/sudoers", os.R_OK):
        sources.append("/etc/sudoers")

    sudoers_d = Path("/etc/sudoers.d")
    if sudoers_d.exists():
        try:
            sources += [
                str(p) for p in sudoers_d.iterdir()
    ...
    for path in sources:
        ...
        for lineno, line in enumerate(content.splitlines(), 1):
            ...
            if "NOPASSWD" in stripped:
                found_nopasswd.append((path, lineno, stripped))
            if re.search(r"ALL\s*=\s*\(ALL", stripped):
                found_all.append((path, lineno, stripped))
...
def check_crontabs(findings: list, verbose: bool = True) -> None:
    header("⏰  CRONTABS")
    ...
    for e in entries:
        is_suspicious = any(
            re.search(p, e["line"], re.IGNORECASE) for p in CRON_SUSPICIOUS
        )
        if is_suspicious:
            flag(f"[SUSPICIOUS] {e['source']}: {e['line']}", "critical")
            findings.append({
                "level": "critical", "module": "crontab",
...
def check_authorized_keys(findings: list, verbose: bool = True) -> None:
    ...
    for u in get_real_users():
        ...
        flag(
            f"{u['username']} – {len(keys)} key(s)  [modified: {age_str}]  {ak_path}",
            "warn" if is_recent else "info",
        )

        if is_recent:
            findings.append({
                "level": "warn", "module": "authorized_keys",
...
SYSTEMD_SUSPICIOUS = re.compile(
    r"(/tmp/|/dev/shm|wget|curl|bash -c|base64|nc )", re.IGNORECASE
)


def check_systemd_units(findings: list, verbose: bool = True) -> None:
    header("⚙️   SYSTEMD USER UNITS")

    search_paths = [
        Path(u["home"]) / ".config" / "systemd" / "user"
        for u in get_real_users()
    ] + [
    ...
    for base in search_paths:
        ...
        for f in service_files:
            ...
            suspicious = any(SYSTEMD_SUSPICIOUS.search(l) for l in exec_lines)
...
def check_rc_files(findings: list, verbose: bool = True) -> None:
    ...
    for u in get_real_users():
        for rc in RC_NAMES:
            ...
            hits = [
                (lineno, line.strip(), desc)
                for lineno, line in enumerate(content.splitlines(), 1)
                for pattern, desc in RC_SUSPICIOUS
...
def show_summary(findings: list) -> None:
